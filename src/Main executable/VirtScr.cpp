...
void VirtualScreen::CopyVSPart(int vx, int vy, int sx, int sy, int SizeX, int SizeY)
{
	...
	// Пошаговое приведение типов
	byte* srcBase = (byte*)VirtualScreenPointer;
	byte* src = srcBase + (vx + vy * RealVLx);
	byte* dstBase = (byte*)ScreenPtr;
	byte* dst = dstBase + (sx + sy * SCRSizeX);
...
//Отказываемся от MMX в пользу более современного решения.
