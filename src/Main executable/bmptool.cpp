...
__declspec(dllexport) void _ExFree(void* ptr) {
	try {
		...
		if (TotalSize > 0) {
			...
			fprintf(stderr, "��������������: TotalSize ��� ����� ����\n");
		}
	}
	catch (const std::exception& e) {
		// ��������� ����� ����������� ����������
		fprintf(stderr, "���������� � _ExFree: %s\n", e.what());
	}
	catch (...) {
		// ��������� ����������� ����������
		fprintf(stderr, "����������� ������ � _ExFree ��� ������ %p\n", ptr);
	}
}
//FIX MEMORY
__declspec(dllexport) void* _ExMalloc(int Size) {
	...
	if (ptr) {
		TotalSize++;
#ifdef _WIN32
		((DWORD*)ptr)[0] = 0xCAFEBABE; // debug marker — only on Windows
#endif
	}
	return ptr;
}